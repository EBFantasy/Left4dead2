class ::AITaskSearchBody extends AITaskGroup
{
	...
	function preCheck() {
		...
		while(deathBody = Entities.FindByClassname(deathBody, "survivor_death_model")) {
			if(BotAI.IsEntityValid(deathBody) && !BotAI.isDeathStillAlive(deathBody) && !(NetProps.GetPropInt(deathBody, "m_nCharacterType") in unreachable) && BotAI.isNearAliveHumanSurvivor(deathBody.GetOrigin(), 600))
	...
	function GroupUpdateChecker(player) {
		...
		local humanAlive = BotAI.hasAliveHumanSurvivor();
		if(humanAlive && !BotAI.isNearAliveHumanSurvivor(player.GetOrigin(), 700))
			return false;

		if(BotAI.IsEntitySurvivor(GUY) && GUY != player)
			return false;

		local distance = humanAlive ? 1200 : 2000;
		local findBody = null;
		foreach(body in fallen) {
			if(!BotAI.IsEntityValid(body) || (humanAlive && !BotAI.isNearAliveHumanSurvivor(body.GetOrigin(), 600)) || BotAI.distanceof(player.GetOrigin(), body.GetOrigin()) > distance)
				continue;
		...
		if(BotAI.HasItem(player, "defibrillator")) {
			...
			local item = null;
			local findDef = null
			local defDis = humanAlive ? 900 : 2000;
			local defRadius = humanAlive ? 900 : 2000;
			while (Entities.FindInSphere(item, player.GetOrigin(), defRadius) != null) {
				item = Entities.FindInSphere(item, player.GetOrigin(), defRadius);
				local name = item.GetClassname();
				local idistance = BotAI.distanceof(player.GetOrigin(), item.GetOrigin());
				if ((name == "weapon_defibrillator" || name == "weapon_defibrillator_spawn") && item.GetOwnerEntity() == null && (!humanAlive || BotAI.isNearAliveHumanSurvivor(item.GetOrigin(), 600))) {
			...
			if(findDef != null) {
				if (defDis > 150) {
					if(!BotAI.IsInCombat(player)) {
						...
						BotAI.botRunPos(player, findDef, "searchBody$", 3, needSearch, humanAlive ? 1200 : 8000);
						return false;
					}
				} else {
					BotAI.doAmmoUpgrades(player);
	...
	function playerUpdate(player) {
		...
		if(ironBanner != null && player != null) {
			local humanAlive = BotAI.hasAliveHumanSurvivor();
			if(humanAlive && (!BotAI.isNearAliveHumanSurvivor(ironBanner.GetOrigin(), 600) || !BotAI.isNearAliveHumanSurvivor(player.GetOrigin(), 800))) {
				ironBanner = null;
				GUY = null;
				BotAI.setBotLockTheard(player, -1);
				return;
			...
			if (distance > (humanAlive ? 1200 : 8000)) {
				ironBanner = null;
				GUY = null;
				BotAI.setBotLockTheard(player, -1);
				return;
			...
			if (distance > 70) {
				...
				if (BotAI.botRunPos(player, ironBanner, "searchBody$", 3, needSearch, humanAlive ? 1200 : 15000)) {
					if(!humanAlive)
