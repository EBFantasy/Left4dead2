	...
	function preCheck() {
		...
		local function searchGen(entT, genType) {
			foreach(player in BotAI.SurvivorBotList) {
				...
				if(BotAI.IsEntitySurvivorBot(player) && !player.IsIncapacitated() && !player.IsHangingFromLedge()
					&& !player.IsDominatedBySpecialInfected() && BotAI.HasItem(player, genType)) {
					stasis[entT] <- genType;
					hasMolotov = true;
				}
			}
		...
		foreach(entT in BotAI.SpecialList) {
			if(BotAI.IsAlive(entT) && entT.GetZombieType() == 8) {
				gettank = true;
				local needVomitjar = BotAI.IsHordePressure(entT, 600.0, 20);
