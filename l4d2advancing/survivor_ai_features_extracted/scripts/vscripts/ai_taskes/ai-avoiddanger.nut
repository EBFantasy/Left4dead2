	...
	function singleUpdateChecker(player) {
		...
		foreach(danger in BotAI.groundList) {
			if (BotAI.IsEntityValid(danger) && BotAI.distanceof(danger.GetOrigin(), player.GetOrigin()) < 270) {
				...
				local abaSpitterOwnsAcid = dangerName == "insect_swarm"
					&& ("ABA_Spitter_ShouldOwnAcid" in getroottable())
					&& ::ABA_Spitter_ShouldOwnAcid(player);

				if (!fireProtect && !acidProtect && !abaSpitterOwnsAcid) {
