...
::BotAI.Events.OnGameEvent_heal_interrupted <- function(event) {
	...
	if(BotAI.IsPlayerEntityValid(bot) && IsPlayerABot(bot)) {
		if(BotAI.isManualKitHealing(bot)
			&& BotAI.manualMedicalHasItem(bot, "kit")
			&& (bot in BotAI.ManualMedical)
			&& Time() <= BotAI.ManualMedical[bot].until) {
			return;
		}
...
::BotAI.Events.OnGameEvent_weapon_fire <- function(event) {
	...
	*/
	...
	if(p != null && p.IsSurvivor() && (IsPlayerABot(p) || BotAI.BotDebugMode)) {
		...
		if(BotAI.IsEntityValid(wep) && !BotAI.isChargerWeaponLockout(p) && (BotAI.getIsMelee(p) || isSniper)) {
			local mult = 0.7;
			if(isSniper) {
				mult = 0.3;
			}
...
::BotAI.Events.OnGameEvent_player_death <- function(event) {
	...
	if(victim != null && victim.IsValid() && victim.GetClassname() == "player" && !victim.IsSurvivor() && victim.GetZombieType() == 6) {
		if(BotAI.BotDebugMode)
			printl("[BotAI][CHARGER] death detected, scanning for trapped bots");
		BotAI.releaseChargerVictims(victim);
	}
}
...
::BotAI.Events.OnGameEvent_charger_carry_start <- function(event) {
	...
	// [预防] 第一行立即进入 charger 武器保护窗, 把 "carry_start -> 事件处理后段"
	// 之间的空窗收为 0 帧. 保护窗内 ABA 不再切枪/攻击/推挤/使用物品, 让引擎独占释放动画和武器重建.
	local mapVictimEarly = GetPlayerFromUserID(event.victim);
	if(BotAI.IsEntitySurvivorBot(mapVictimEarly))
		BotAI.beginChargerWeaponLockout(mapVictimEarly, 4.5);

	// [CHARGER] 记录 charger->victim 映射: charger 在抓取瞬间被击杀时, 引擎可能抢先清掉
	// 双方 netprop, player_death 里的扫描会漏人 -> 靠这张映射兜底定位受害者
	local mapCharger = GetPlayerFromUserID(event.userid);
	local mapVictim = GetPlayerFromUserID(event.victim);
	if(mapCharger != null && mapVictim != null)
		BotAI.chargerVictimMap[mapCharger.GetEntityIndex()] <- mapVictim;
...
::BotAI.Events.OnGameEvent_charger_pummel_start <- function(event) {
	...
	local victim = GetPlayerFromUserID(event.victim);
	if(BotAI.IsEntitySurvivorBot(victim))
		BotAI.beginChargerWeaponLockout(victim, 4.5);

	// [CHARGER] 与 carry_start 同理, 碾压阶段也维护映射
	local mapCharger = GetPlayerFromUserID(event.userid);
	if(mapCharger != null && victim != null)
		BotAI.chargerVictimMap[mapCharger.GetEntityIndex()] <- victim;
...
::BotAI.Events.OnGameEvent_charger_impact <- function(event) {
	...
	if(BotAI.IsEntitySurvivorBot(victim)) {
		BotAI.beginChargerWeaponLockout(victim, 4.5);
		if(BotAI.BotDebugMode)
			printl("[BotAI][CHARGER] impact lockout for " + BotAI.getPlayerBaseName(victim));
	}

	if(("userid" in event) && event.userid != null) {
		local charger = GetPlayerFromUserID(event.userid);
		if(charger != null && victim != null)
			BotAI.chargerVictimMap[charger.GetEntityIndex()] <- victim;
	}
}

::BotAI.Events.OnGameEvent_charger_carry_end <- function(event) {
	if(!("victim" in event) || event.victim == null)
		return;
	...
	BotAI.beginChargerWeaponLockout(victim, 4.5);
	if(BotAI.BotDebugMode)
		printl("[BotAI][CHARGER] carry_end lockout for " + BotAI.getPlayerBaseName(victim));
}
...
::BotAI.Events.OnGameEvent_charger_pummel_end <- function(event) {
	if(!("victim" in event) || event.victim == null)
		return;
	local victim = GetPlayerFromUserID(event.victim);
	if(!BotAI.IsPlayerEntityValid(victim))
		return;

	BotAI.SurvivorTrapped[victim.GetEntityIndex()] <- null;
	BotAI.SurvivorTrappedTimed[victim.GetEntityIndex()] <- null;

	// [CHARGER] 正常释放 -> 清映射, 防止 charger 之后死亡时对老受害者重复 scrub
	if(("userid" in event) && event.userid != null) {
		local mapCharger = GetPlayerFromUserID(event.userid);
		if(mapCharger != null && mapCharger.GetEntityIndex() in BotAI.chargerVictimMap)
			delete BotAI.chargerVictimMap[mapCharger.GetEntityIndex()];
	}

	if(BotAI.IsEntitySurvivorBot(victim)) {
		BotAI.beginChargerWeaponLockout(victim, 4.5);
		if(BotAI.BotDebugMode)
			printl("[BotAI][CHARGER] pummel_end lockout 4.5s for " + BotAI.getPlayerBaseName(victim));
	}
}

::BotAI.releaseChargerVictims <- function(charger) {
	if(charger == null || !charger.IsValid())
		return;

	local function scrub(p) {
		if(!BotAI.IsEntitySurvivorBot(p)) return;
		local idx = p.GetEntityIndex();

		// charger 死亡事件可能发生在释放动画还没建好之前. 这里先只让 ABA 停手,
		// 不立刻清支配 prop/改攻击计时器, 避免和引擎的 Charger_Slammed_* 重建流程抢状态.
		BotAI.beginChargerWeaponLockout(p, 4.5);

		local function postReleaseScrub(args) {
			local b = args.b;
			if(!BotAI.IsEntitySurvivorBot(b)) return;
			BotAI.clearStaleChargerLock(b, true);
			BotAI.beginChargerWeaponLockout(b, 2.0);
			printl("[BotAI][CHARGER] delayed post-release scrub for " + BotAI.getPlayerBaseName(b));
		}
		BotAI.Timers.AddTimerByName("ChargerPostReleaseScrub" + idx, 2.6, false, postReleaseScrub, {b = p});
		printl("[BotAI][CHARGER] released victim " + BotAI.getPlayerBaseName(p) + " (charger died mid-carry), ABA weapon/action lockout 4.5s");
		// [兜底] 启动释放后监视器: 缓冲期只观察; 缓冲期过后仍卡死 -> 软修复 -> 硬重置 (最终保险)
		BotAI.startChargerDiag(p);
	}

	// 路径 1: netprop 扫描 (charger 死亡时引擎还没清 netprop 的场景)
	local p = null;
	while(p = Entities.FindByClassname(p, "player")) {
		if(!BotAI.IsEntitySurvivorBot(p))
			continue;
		local carry = NetProps.GetPropEntity(p, "m_carryAttacker");
		local pummel = NetProps.GetPropEntity(p, "m_pummelAttacker");
		if(carry == charger || pummel == charger)
			scrub(p);
	}

	// 路径 2: 映射兜底 (引擎抢先清掉 netprop 时, 扫描漏人 -> 用 carry/pummel_start 记的映射)
	local cidx = charger.GetEntityIndex();
	if(cidx in BotAI.chargerVictimMap) {
		local v = BotAI.chargerVictimMap[cidx];
		if(BotAI.IsEntityValid(v))
			scrub(v);
		delete BotAI.chargerVictimMap[cidx];
	}
}
...
::BotAI.Events.OnGameEvent_player_shoved <- function(event) {
	...
	if(BotAI.IsPlayerEntityValid(attacker) && BotAI.IsPlayerEntityValid(victim))
		BotAI.HandleSupplySwapUseShove(attacker, victim);
}
...
::BotAI.Events.OnGameEvent_create_panic_event <- function(event) {
	BotAI.MarkHordePressure(45.0);
}

::BotAI.Events.OnGameEvent_triggered_car_alarm <- function(event) {
	BotAI.MarkHordePressure(45.0);
}

::BotAI.Events.OnGameEvent_panic_event_finished <- function(event) {
	BotAI.ClearHordePressure();
}
...
::BotAI.Events.OnGameEvent_entity_shoved <- function(event) {
	...
	if (BotAI.IsPlayerEntityValid(attacker) && BotAI.HandleSupplySwapUseShove(attacker))
		return;
...
function VSLib::EasyLogic::Notifications::CanPickupObject::BotAIPickUp(vEntity, className) {
	...
	foreach(idx, thing in BotAI.BotLinkGasCan) {
		if(entity == thing) {
			BotAI.blockBackpackPickup(entity, 5.0);
...
function VSLib::EasyLogic::OnTakeDamage::BotAITakeDamage(damageTable) {
	...
	// [WATCHDOG-KILL 白名单] hardResetBot 处死 bot 时挂的旁路标记: 标记下的 victim 被无条件放行,
	// 绕过本钩子下面所有 friendly-fire/Immunity/NonAliveProtect/FallProtect 等抹零规则.
	// (self-attacker 仍会被 1444 行的 friendly-fire 拦, 必须用白名单才能真处死.)
	// hardResetBot 在 TakeDamage 调用前后自己负责设置/清除该标记.
	if(victim != null && BotAI.IsPlayerEntityValid(victim) && victim.GetEntityIndex() in BotAI.WatchdogKilling)
		return true;
