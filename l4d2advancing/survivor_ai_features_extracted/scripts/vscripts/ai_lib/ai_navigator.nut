			...
			foreach(danger in BotAI.SpecialList) {
				if (danger.GetClassname() == "player" && danger.GetZombieType() == 8) {
					...
					if (BotAI.distanceof(danger.GetOrigin(), player.GetOrigin()) < 200) {
						speed += 0.2 + botskill * 0.15;
						friction -= 0.07 * botskill;
					}
				}
			}
		}

		local isManualScoutMove = movingID == "manualScout$";
		local isAutonomousLeadMove = movingID == "manualLeadAuto$";
		local isManualLeadMove = movingID == "manualLead$" || isManualScoutMove || isAutonomousLeadMove;
		if (movingID.find("$") != null && !isManualLeadMove) {
			speed += 0.1 * BotAI.BotCombatSkill;
		}
		if (movingID == "manualLead$" || isManualScoutMove) {
			speed = 1.0;
			friction = 1.0;
		} else if (isAutonomousLeadMove) {
			local autonomousLeadCombat = nearTank || BotAI.IsInCombat(player);
			if (!autonomousLeadCombat && player in BotAI.dangerInfected && BotAI.IsEntityValid(BotAI.dangerInfected[player])) {
				autonomousLeadCombat = true;
			}
			if (!autonomousLeadCombat) {
				speed = 1.0;
				friction = 1.0;
			} else if (!nearTank) {
				local combatBonus = 1.0 + 0.08 * botskill;
				if (speed < combatBonus) {
					speed = combatBonus;
				}
			}
		}
