...
::BotPathFindingCmd <- function ( speaker, args , args1) {
	...
	if(BotAI.PathFinding) {
		...
		Convars.SetValue( "sb_allow_leading", 0 ); // [语音] 解绑: 保留寻路功能, 但不让引擎播领队嘴炮
...
function BotAI::registerMenu() {
	...
	local function openMenu(player) {
		...
		if(player in BotAI.pingMedicalBot)
			delete BotAI.pingMedicalBot[player];
	...
	local function filterIcon(player, functionsIn) {
		...
		if(typeof entity == "Vector") {
			foreach(idx, func in functionsIn) {
				if(idx == "ping_move" || idx == "ping_stay" || idx == "ping_follow_me" || idx == "ping_teleport" || idx == "ping_medical" || idx == "ping_manual_use")
					functions[idx] <- func;
			}
			...
			if(name == "player" || name == "infected" || name == "witch") {
				foreach(idx, func in functionsIn) {
					if(idx == "ping_move" || idx == "ping_attack" || idx == "ping_follow" || idx == "ping_medical")
						functions[idx] <- func;
				}
			} else {
				foreach(idx, func in functionsIn) {
					if(idx == "ping_move" || idx == "ping_use" || idx == "ping_stay" || idx == "ping_attack" || idx == "ping_medical" || idx == "ping_manual_use")
						functions[idx] <- func;
				}
			}
		}
		...
		if("ping_lead" in functionsIn)
			functions["ping_lead"] <- functionsIn["ping_lead"];
		if("ping_attack" in functionsIn && !("ping_attack" in functions))
			functions["ping_attack"] <- functionsIn["ping_attack"];
		if("ping_scout" in functionsIn)
			functions["ping_scout"] <- functionsIn["ping_scout"];
		if("ping_teleport" in functionsIn)
			functions["ping_teleport"] <- functionsIn["ping_teleport"];
		if("ping_medical" in functionsIn)
			functions["ping_medical"] <- functionsIn["ping_medical"];
	...
	local function cancelScoutCommand(bot) {
		if(!BotAI.IsEntitySurvivorBot(bot))
			return;

		if(bot in BotAI.ManualLead && "mode" in BotAI.ManualLead[bot] && BotAI.ManualLead[bot].mode == "scout")
			BotAI.stopManualLead(bot);
	}

	local function isManualCommandLocked(bot) {
		if(!BotAI.IsEntitySurvivorBot(bot))
			return false;

		return (bot in BotAI.ManualMedical) || (bot in BotAI.ManualUse);
	}

	local function stopIfManualCommandLocked(player, bot) {
		if(isManualCommandLocked(bot)) {
			if(player in BotAI.pingPoint)
				delete BotAI.pingPoint[player];
			return true;
		}
	...
	local function move(player, args = "", args1 = "") {
		...
		local entity  = BotAI.pingEntity[player];
		if(player in BotAI.pingPoint) {
			if(stopIfManualCommandLocked(player, BotAI.pingPoint[player]))
				return;
			cancelScoutCommand(BotAI.pingPoint[player]);
		}
	...
	local function use(player, args = "", args1 = "") {
		...
		player = convertPlayer(player);
		local entity  = BotAI.pingEntity[player];
		if(!(player in BotAI.pingPoint)) return;
		local bot = BotAI.pingPoint[player];
		if(stopIfManualCommandLocked(player, bot))
			return;
		cancelScoutCommand(bot);
	...
	local function sendSyncAttackState(player, bot, enabled) {
		local message = enabled ? "Sync attack enabled" : "Sync attack disabled";
		if(BotAI.language == "schinese")
			message = enabled ? "同步攻击已启用" : "同步攻击已取消";
		else if(BotAI.language == "tchinese")
			message = enabled ? "同步攻擊已啟用" : "同步攻擊已取消";

		BotAI.SendPlayerNoPrefix(player, message + ": " + BotAI.getPlayerBaseName(bot));
	}
	...
	local function attack(player, args = "", args1 = "") {
		...
		if(stopIfManualCommandLocked(player, bot))
			return;
		cancelScoutCommand(bot);
		if(bot in BotAI.syncAttack) {
			delete BotAI.syncAttack[bot];
			sendSyncAttackState(player, bot, false);
		} else {
			if(bot in BotAI.targetLocked)
				delete BotAI.targetLocked[bot];
			BotAI.syncAttack[bot] <- player;
			sendSyncAttackState(player, bot, true);
		}
		delete BotAI.pingPoint[player];
	}
	...
	local function getPingHint(player) {
		if(player in BotAI.pingEntity) {
			local entity = BotAI.pingEntity[player];
			if(typeof entity == "Vector")
				return entity;
	...
	local function getPingHintMeta(player) {
		local meta = {
			eyePos = player.EyePosition()
			forward = player.EyeAngles().Forward()
		};

		if(player in BotAI.pingEntity) {
			local entity = BotAI.pingEntity[player];
			if(typeof entity != "Vector" && BotAI.IsEntityValid(entity))
				meta.hitEntity <- entity;
		}

		return meta;
	}

	local function lead(player, args = "", args1 = "") {
		BotExitMenuCmd(player, args, args1);
		player = convertPlayer(player);
		if(!(player in BotAI.pingPoint)) return;
		...
		if(bot in BotAI.ManualLead && "mode" in BotAI.ManualLead[bot] && BotAI.ManualLead[bot].mode == "lead") {
			BotAI.stopManualLead(bot);
			BotAI.SendPlayerNoPrefix(player, "Lead stopped: " + BotAI.getPlayerBaseName(bot));
		} else {
			if(stopIfManualCommandLocked(player, bot))
				return;
			cancelScoutCommand(bot);
			BotAI.startManualLead(bot, null, "lead", 0.0, null, null, true);
			BotAI.SendPlayerNoPrefix(player, "Lead enabled: " + BotAI.getPlayerBaseName(bot));
		}

		delete BotAI.pingPoint[player];
	}

	local function scout(player, args = "", args1 = "") {
		BotExitMenuCmd(player, args, args1);
		player = convertPlayer(player);
		if(!(player in BotAI.pingPoint)) return;
		...
		local hint = getPingHint(player);
		if(stopIfManualCommandLocked(player, bot))
			return;
		BotAI.startManualLead(bot, player, "scout", 30.0, hint, getPingHintMeta(player));
		BotAI.SendPlayerNoPrefix(player, "Scout 30s: " + BotAI.getPlayerBaseName(bot));
		delete BotAI.pingPoint[player];
	}
	...
	local function teleport(player, args = "", args1 = "") {
		...
		if(player in BotAI.pingPoint) {
			if(typeof pos != "Vector" && BotAI.IsEntityValid(pos))
				pos = pos.GetOrigin();
		}
	...
	local function manualUse(player, args = "", args1 = "") {
		BotExitMenuCmd(player, args, args1);
		player = convertPlayer(player);
		if(!(player in BotAI.pingPoint)) return;
		...
		local pos = null;
		if(player in BotAI.pingEntity) {
			local ping = BotAI.pingEntity[player];
			if(typeof ping == "Vector")
				pos = ping;
			else if(BotAI.IsEntityValid(ping)) {
				entity = ping;
				pos = ping.GetOrigin();
			}
		}

		if(BotAI.startManualUse(bot, entity, pos, player))
			BotAI.SendPlayerNoPrefix(player, I18n.getTranslationKeyByLang(BotAI.language, "ping_manual_use") + ": " + BotAI.getPlayerBaseName(bot));

		delete BotAI.pingPoint[player];
	}

	local function medicalKit(player, args = "", args1 = "") {
		BotExitMenuCmd(player, args, args1);
		player = convertPlayer(player);
		if(!(player in BotAI.pingMedicalBot)) return;
		local bot = BotAI.pingMedicalBot[player];
		if(BotAI.startManualMedical(bot, "kit", player))
			BotAI.SendPlayerNoPrefix(player, I18n.getTranslationKeyByLang(BotAI.language, "ping_medical_kit") + ": " + BotAI.getPlayerBaseName(bot));
		delete BotAI.pingMedicalBot[player];
		if(player in BotAI.pingPoint) delete BotAI.pingPoint[player];
	}

	local function medicalPills(player, args = "", args1 = "") {
		BotExitMenuCmd(player, args, args1);
		player = convertPlayer(player);
		if(!(player in BotAI.pingMedicalBot)) return;
		local bot = BotAI.pingMedicalBot[player];
		if(BotAI.startManualMedical(bot, "pills", player))
			BotAI.SendPlayerNoPrefix(player, I18n.getTranslationKeyByLang(BotAI.language, "ping_medical_pills") + ": " + BotAI.getPlayerBaseName(bot));
		delete BotAI.pingMedicalBot[player];
		if(player in BotAI.pingPoint) delete BotAI.pingPoint[player];
	}

	local function medicalAdrenaline(player, args = "", args1 = "") {
		BotExitMenuCmd(player, args, args1);
		player = convertPlayer(player);
		if(!(player in BotAI.pingMedicalBot)) return;
		local bot = BotAI.pingMedicalBot[player];
		if(BotAI.startManualMedical(bot, "adrenaline", player))
			BotAI.SendPlayerNoPrefix(player, I18n.getTranslationKeyByLang(BotAI.language, "ping_medical_adrenaline") + ": " + BotAI.getPlayerBaseName(bot));
		delete BotAI.pingMedicalBot[player];
		if(player in BotAI.pingPoint) delete BotAI.pingPoint[player];
	}

	local function medicalCancel(player, args = "", args1 = "") {
		BotExitMenuCmd(player, args, args1);
		player = convertPlayer(player);
		if(player in BotAI.pingMedicalBot)
			delete BotAI.pingMedicalBot[player];
	...
	local function medicalBack(player, args = "", args1 = "") {
		BotExitMenuCmd(player, args, args1);
		player = convertPlayer(player);
		if(player in BotAI.pingMedicalBot) {
			BotAI.pingPoint[player] <- BotAI.pingMedicalBot[player];
			delete BotAI.pingMedicalBot[player];
			openMenu(player);
		}
	}

	local function displayPingMedicalMenu(player, args = "", args1 = "") {
		BotExitMenuCmd(player, args, args1);
		player = convertPlayer(player);
		if(!(player in BotAI.pingPoint) || !BotAI.IsEntitySurvivorBot(BotAI.pingPoint[player]))
			return;

		BotAI.pingMedicalBot[player] <- BotAI.pingPoint[player];
		local lang = BotAI.language;
		local function top(menu) {
			menu.AddOption(I18n.getTranslationKeyByLang(lang, "ping_medical_kit"), medicalKit);
			menu.AddOption(I18n.getTranslationKeyByLang(lang, "ping_medical_pills"), medicalPills);
			menu.AddOption(I18n.getTranslationKeyByLang(lang, "ping_medical_adrenaline"), medicalAdrenaline);
			menu.AddOption("emp_0", BotEmptyCmd);
			menu.AddOption("emp_1", BotEmptyCmd);
		}
		...
		local function bot(menu) {
			...
			menu.AddOption(I18n.getTranslationKeyByLang(lang, "ping_medical_back"), medicalBack);
			menu.AddOption(I18n.getTranslationKeyByLang(lang, "ping_medical_cancel"), medicalCancel);
		}
	...
	pingMenu.registerFunction("ping_lead", lead);
	pingMenu.registerFunction("ping_scout", scout);
	pingMenu.registerFunction("ping_medical", displayPingMedicalMenu);
	pingMenu.registerFunction("ping_manual_use", manualUse);
