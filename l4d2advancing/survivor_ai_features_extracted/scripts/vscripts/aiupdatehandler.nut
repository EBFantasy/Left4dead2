...
//if (!("BotAI" in getroottable()))
{
	::BotAI <- {
		...
		somethingBad = {}
		waitingToPick = {}
		BackpackPickupBlockUntil = {}
		BackpackPickupAreaBlock = {}
		BackpackPickupAreaNextId = 0
		BackpackPickupWatch = {}
		ManualLead = {}
		preTake = {}
		...
		WatchdogLastReset = {}
		WatchdogLastActive = {}
		WatchdogLastNextAttack = {}
		WatchdogProgressTime = {}
		WatchdogAtkAttempts = {}
		WatchdogHealStart = {}
		WatchdogHardLast = {}
		WatchdogResetCount = {}
		...
		chargerVictimMap = {}
		SharedVisionTargets = {}
		SharedVisionBotTargets = {}
		SharedVisionNextScan = 0.0
		AssistHumanTargets = {}
		ManualMedical = {}
		ManualUse = {}
...
IncludeScript("ai_lib/ai_context_features.nut");
...
IncludeScript("ai_taskes/ai-sharedVisionAssist.nut");
IncludeScript("ai_taskes/ai-manualCommands.nut");
...
::BotAI.getBotRetreatCooldown <- function(player) {
	if(player != null && !("BotRetreatCooldown" in BotAI))
		BotAI.BotRetreatCooldown <- {};

	if(player != null && player.GetEntityIndex() in BotAI.BotRetreatCooldown)
		return BotAI.BotRetreatCooldown[player.GetEntityIndex()];

	return 0;
}

::BotAI.setBotRetreatCooldown <- function(player, cooldown) {
	if(player == null)
		return;

	if(!("BotRetreatCooldown" in BotAI))
		BotAI.BotRetreatCooldown <- {};

	BotAI.BotRetreatCooldown[player.GetEntityIndex()] <- cooldown;
}
...
function BotAI::AdjustBotsUpdateRate(args) {
	...
	foreach(playerSet in BotAI.SurvivorList) {
		if(BotAI.IsPlayerEntityValid(playerSet)) {
			...
			if(IsPlayerABot(playerSet) || playerSet in BotAI.humanBot) {
				...
				if(BotAI.isManualKitHealing(playerSet)) {
					if(BotAI.shouldPauseManualKit(playerSet)) {
						BotAI.RemoveFlag(playerSet, FL_FROZEN);
						BotAI.UnforceButton(playerSet, 1);
					} else {
						BotAI.protectManualKitAction(playerSet, Convars.GetFloat("first_aid_kit_use_duration") + 0.5);
					}
				} else if (!BotAI.isChargerWeaponLockout(playerSet) && weapon && weapon.GetClassname() == "weapon_first_aid_kit") {
					if (canRest) {
				...
				if (!BotAI.isChargerWeaponLockout(playerSet) && weapon && BotAI.IsEntitySurvivor(t) && (weapon.GetClassname() != "weapon_first_aid_kit" && weapon.GetClassname() != "weapon_pain_pills" && weapon.GetClassname() != "weapon_adrenaline")) {
	...
	foreach(idx, gas in BotAI.BotLinkGasCan) {
		if(!BotAI.IsEntityValid(gas)) {
			...
			BotAI.blockBackpackPickup(gas, 5.0);
			delete BotAI.BotLinkGasCan[idx];
		} else {
...
function BotAI::AdjustBotState(args) {
	...
	if (BotAI.SurvivorHumanList.len() > 0) {
		...
		if(BotAI.PathFinding) {
			Convars.SetValue( "sb_allow_leading", 0 ); // [语音] 解绑: 寻路开启时也不播领队嘴炮
		} else {
		...
		local function using() {
			...
			if (BotAI.IsEntityValid(gascan)) {
				...
				if (BotAI.IsEntitySurvivor(player)) {
					...
					} else if(!BotAI.isChargerWeaponLockout(player)) {
...
function BotAI::loadAITask() {
	...
	BotAI.addTask("manualMedical", AITaskManualMedical(1, 1, true, true));
	BotAI.addTask("manualUse", AITaskManualUse(1, 1, true, true));
	BotAI.addTask("sharedVisionAssist", AITaskSharedVisionAssist(1, 2, true, true));
...
::BotAI.Events.OnGameEvent_round_start_post_nav <- function(event) {
	...
	BotAI.SupplySwapHandledUntil.clear();
